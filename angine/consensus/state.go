...
import (
	...
	bc "github.com/annchain/annchain/angine/blockchain"
	mempl "github.com/annchain/annchain/angine/mempool"
	csspb "github.com/annchain/annchain/angine/protos/consensus"
	pbtypes "github.com/annchain/annchain/angine/protos/types"
	sm "github.com/annchain/annchain/angine/state"
...
func (cs *ConsensusState) createProposalBlock() (block *agtypes.BlockCache, blockParts *agtypes.PartSet) {
	...
	return agtypes.MakeBlock(proposerPubkey[:], cs.privValidator.GetCoinbase(),
		cs.Height, cs.state.ChainID, txs, commit,
		cs.state.LastBlockID, cs.state.Validators.Hash(), cs.Validators,
		cs.state.AppHash, cs.state.ReceiptsHash,
		cs.config.GetInt64("block_part_size"), cs.state.LastNonEmptyHeight)
}
...
func (cs *ConsensusState) defaultDoPrevote(height, round def.INT) {
	...
	txs := agtypes.Txs(agtypes.BytesToTxSlc(cs.ProposalBlock.Data.Txs))
	// Check block size
	if cs.config.GetInt("block_max_size") < getBlockTxTotalSize(txs) {
		// ProposalBlock is over-size, prevote nil.
		cs.logger.Warn("enterPrevote: ProposalBlock is over size")
		cs.signAddVote(pbtypes.VoteType_Prevote, nil, &pbtypes.PartSetHeader{})
		return
	}
...
func getBlockTxTotalSize(txs agtypes.Txs) int {
	n := 0
	for _, v := range txs {
		n += len(v)
	}
...
func (cs *ConsensusState) enterPrecommit(height, round def.INT) {
	...
	//	cs.signAddVote(agtypes.VoteTypePrecommit, nil, agtypes.PartSetHeader{})
	//	return
	// }
...
func (cs *ConsensusState) finalizeCommit(height def.INT) {
	...
	// eventCache := agtypes.NewEventCache(cs.evsw)
...
func (cs *ConsensusState) signAddVote(type_ pbtypes.VoteType, hash []byte, header *pbtypes.PartSetHeader) *pbtypes.Vote {
	...
	if err == nil {
		...
		if !cs.replayMode {
			cs.slogger.Warnf("Error signing vote height %d, round %d, vote %v, error %v", cs.Height, cs.Round, vote.GetData().GetBlockID().Key(), err)
		}
		return nil
	}
}
