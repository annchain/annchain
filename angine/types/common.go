...
func PubkeyToPbData(pubkey crypto.PubKey) *pbtypes.PubKey {
	var pk pbtypes.PubKey
	switch pubkey.(type) {
	case (*crypto.PubKeyEd25519):
		pk.Type = pbtypes.KeyType_Ed25519
	case (*crypto.PubKeySecp256k1):
		pk.Type = pbtypes.KeyType_Secp256k1
	}
	pk.Bytes = pubkey.Bytes()
	return &pk
}

func PbDataToPubkey(pubkey *pbtypes.PubKey) crypto.PubKey {
	switch pubkey.Type {
	case pbtypes.KeyType_Ed25519:
		pk := crypto.PubKeyEd25519(BytesToByte32(pubkey.Bytes))
		return &pk
	case pbtypes.KeyType_Secp256k1:
		pk := crypto.PubKeySecp256k1(BytesToByte64(pubkey.Bytes))
		return &pk
	default:
		return nil
	}
