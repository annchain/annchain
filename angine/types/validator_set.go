...
func (vs *ValidatorSet) ToPbVSet() *pbtypes.ValidatorSet {
	vset := make([]*pbtypes.Validator, len(vs.Validators))
	for i := range vs.Validators {
		vset[i] = &pbtypes.Validator{
			Address:     vs.Validators[i].Address,
			Pubkey:      PubkeyToPbData(vs.Validators[i].GetPubKey()),
			VotingPower: vs.Validators[i].VotingPower,
			Accum:       vs.Validators[i].Accum,
			IsCA:        vs.Validators[i].IsCA,
		}
	}
	return &pbtypes.ValidatorSet{vset}
}

func ValSetFromPb(vslc *pbtypes.ValidatorSet) *ValidatorSet {
	vals := make([]*Validator, len(vslc.VSet))
	for i := range vslc.VSet {
		pubk := PbDataToPubkey(vslc.VSet[i].Pubkey)
		vals[i] = &Validator{
			Address:     vslc.VSet[i].Address,
			PubKey:      crypto.StPubKey{pubk},
			VotingPower: vslc.VSet[i].VotingPower,
			Accum:       vslc.VSet[i].Accum,
			IsCA:        vslc.VSet[i].IsCA,
		}
	}
...
func (valSet *ValidatorSet) CopyWithoutAccum() *ValidatorSet {
	...
	for i, val := range valSet.Validators {
		// NOTE: must copy, since IncrementAccum updates in place.
		validators[i] = val.CopyWithoutAccum()
	}
	return &ValidatorSet{
		Validators:       validators,
		proposer:         valSet.proposer,
